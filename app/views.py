...
from app.models import Post, InstaKiloGramUser, Comment
...
class UserPublicProfile(DetailView):
    model = InstaKiloGramUser
    template_name = 'app/user_public_profile.html'

    def post(self, request, *args, **kwargs):
        profile_user = InstaKiloGramUser.objects.get(id=request.POST.get('user_id'))
...
class SearchUsers(ListView):
    ...
    def get_queryset(self):
        try:
            ...
            if len(query) < 3:
                ...
                users_list = InstaKiloGramUser.objects.exclude(username=self.request.user.username).filter(
    ...
    def post(self, request, *args, **kwargs):
        result_user = InstaKiloGramUser.objects.get(id=request.POST.get('user_id'))
...
class UserFollowingList(ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        followed_user = InstaKiloGramUser.objects.get(id=user_id)
